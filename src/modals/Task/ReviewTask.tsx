...
export default function ReviewTask({ taskInformation, workRequestInformation, setStep }: ReviewTaskProps) {
  ...
  const isWorkOrder = !!taskInformation;
  ...
  const TaskFields = {
    TaskType: isWorkOrder
      ? taskInformation?.workOrderType?.workOrderTypeName
  ...
  return (
    <ScrollView>
      {!taskInformation?.loading ? <View style={{ marginTop: 20, paddingHorizontal: 16 }}>
        {/* Requestor Info */}
      ...
      </View> : <ActivityIndicator style={{marginVertical: "50%"}} size={"small"} color={colors.ACTIVE_ORANGE} />}
    </ScrollView>
  );
}
